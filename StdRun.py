...
mld_sinu[:, 1] = (-((Zmix-ThermoZ)/2+ThermoZ) + (Zmix-ThermoZ)/2 * np.cos(np.arange(365)/4.825)) * -1
    # medium mixing frequency
mld_sinu[:, 2] = (-((Zmix-ThermoZ)/2+ThermoZ) + (Zmix-ThermoZ)/2 * np.cos(np.arange(365) / 14.525)) * -1
...
for i in range(numMixRgm):
    axs.plot(np.arange(365), mld_sinu[:, i], label=lab[i])
axs.set_ylim(85, 0)
axs.set_xlim(0, 365)
axs.set_xlabel('Time [Month]', fontsize=12)
axs.set_ylabel('Mixing depth [meter]', fontsize=12)
axs.set_xticks(np.linspace(0, 365, 12))
axs.set_xticklabels(['J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D'])
axs.legend(loc=1, bbox_to_anchor=(0.85, 1.12), ncol=3)
"""
